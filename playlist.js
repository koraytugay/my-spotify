...
function createTrackCard(t) {
    ...
    card.innerHTML = `
        ...
        <div class="song-meta" style="justify-content: space-between; align-items: center;">
            <button class="album-card-btn album-card-play-btn" onclick="togglePlayPreview('${t.id}')" title="Play Track" aria-label="Play Track">
                ▶
            </button>
...
function togglePlayPreview(id) {
    if (window.miniPlayer) {
        const track = (filteredTracks || []).find(t => t.id === id) || (allTracks || []).find(t => t.id === id) || { id };
        window.miniPlayer.currentType = 'playlist';
        window.miniPlayer.contextTitle = (typeof playlistData !== 'undefined' && playlistData?.name) ? playlistData.name : 'Playlist';
        window.miniPlayer.playTrack(track, filteredTracks);
    }
}
